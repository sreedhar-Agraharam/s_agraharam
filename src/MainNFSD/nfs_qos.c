...
/* max string lengths for the client address and max values*/
#define QOS_MAX_ADDR_STR_LEN 48 // Sufficient for IPv6 + port (e.g., "[::1]:65535\0")
#define QOS_MAX_LONG_STR_LEN 22 // Sufficient for largest signed long long (approx -9.22e18) + null terminator
...
/* q_qos_lock is global lock which is used for below scenarios.
 * Allocation of new QOS structure in IO path.
 * 1. PerExport case      : Global qos values are there in config,
 *			    but export level config not present, so populate
 *			    value at runtime for this share in IO path.
 * 2. PerClient case      : Runtime client QOS value population from
 *			    Global config in IO path
 * 3. PerExport-PerClient : Runtime addition of new clients QOS limit
 *			    i.e allocate new qos_class_t and add in share list
 * 4. QOS threads intialization/ QOS thread shutdown sync.
 * */
pthread_mutex_t g_qos_iopath_lock;

/* Lock meant for allocation of qos_class_t
 * So from IO path if config is getting added, user should first hold
 * the g_qos_iopath_lock before calling the perExportInsert or PerClientInsert
 * which internally holds this lock, syncing between:
 * -> IO_PATH <-> dbus <-> Config_reload update <-
 * */
pthread_mutex_t g_qos_config_lock;

pthread_t qos_thread[2];
/* Flag to control QOS thread lifecycle */
static bool qos_thread_running;
 ...
 */
qos_class_t *allocate_qos_class(unsigned int class_type)
{
	qos_class_t *node = gsh_calloc(1, sizeof(qos_class_t));
	node->type = class_type;
	node->enabled_metric = false;
...
get_and_insert_client_details(struct glist_head *head, compound_data_t *data)
{
	...
	glist_for_each(glist, head) {
		current = glist_entry(glist, qos_client_entry_t, glist);
		if (current->gsh_client == op_ctx->client)
			return current;
	}

	new_entry = alloc_clientdetails_pe(data);
	glist_add_tail(head, &new_entry->glist);
	return new_entry;
}
 ...
 *
 * @return [out] Ptr to new glist entry, this is done so that caller will
 *		send the current glist as head to next entry addition,
 *		which will save iterate again from start of list.
 */
					     ...
					     timer_entry_t *new_entry)
{
	...
	glist_for_each(glist, head) {
		current = glist_entry(glist, timer_entry_t, glist);
		if (current->expiry > new_entry->expiry) {
			glist_add(glist->prev, &new_entry->glist);
			return &new_entry->glist;
		}
	}
	glist_add_tail(head, &new_entry->glist);
	return &new_entry->glist;
 ...
 * caller should hold bucket->lock to which list belongs.
 *
 * @param [in] head Pointer to the head of the waitlist
 *	  head Pointer can be :
 *		1. Token Exhausted waitlist.
 ...
 * caller should hold bucket->lock to which timer belongs.
 *
				    ...
				    unsigned int *counter2)
{
	...
	glist_for_each_safe(glist, glistn, head) {
		entry = glist_entry(glist, timer_entry_t, glist);
 ...
 * Caller should hold export->lock.
 *
...
/**
 * @brief Get the qos_class_t associated with a client.
 * This function retrieves qos_class_t for a given gsh_client structure.
 *
 * @param [in] client Pointer to the gsh_client structure
 *
 * @return qos_class_t pointer to QoS client class, or NULL if not found
 */
qos_class_t *get_client_qos(struct gsh_client *gsh_client)
{
	qos_class_t *qos_class = NULL;

	if (gsh_client && gsh_client->qos_class)
...
/**
 * @brief Get the qos_class_t associated with an export.
 *
 * @param [in] gsh_export Pointer to the gsh_export structure
 *
 * @return qos_class_t pointer to QoS export class, or NULL if not found
 */
qos_class_t *get_export_qos(struct gsh_export *gsh_export)
{
	qos_class_t *qos_class = NULL;

	if (gsh_export && gsh_export->qos_class)
 ...
 * Caller should hold the qos_class->lock.
 *
 * @param [in] qos_class Pointer to the QoS export or client object
 */
static inline void qos_drain_token_ios(qos_class_t *qos_class)
{
	...
	if (token_enabled && !glist_empty(client_list)) {
		...
		glist_for_each_safe(glist, glistn, client_list) {
			token_client =
				glist_entry(glist, qos_client_entry_t, glist);
...
void qos_free_mem(void *gsh_ptr, unsigned int qos_class_type)
{
	...
	case QOS_PER_CLIENT_ENABLED:
		if (qos_class_type == QOS_CLIENT && client->qos_class) {
...
static void register_metric(unsigned int class_type, char *sub_type,
			    char *rd_wr, struct gauge_metric_handle *handle,
			    metric_label_t *lable, uint64_t value,
			    char *max_value)
{
	char label_key[MAX_LABEL_KEY_LEN];
	char *type = (class_type == QOS_EXPORT) ? "export" : (class_type == QOS_CLIENT) ? "client" : "pepc";
	
	snprintf(label_key, MAX_LABEL_KEY_LEN, "max_%s_of_%s", sub_type,
		 type );
	lable[1] = METRIC_LABEL(label_key, max_value);

	char meta_data_key[MAX_METADATA_KEY_LEN];
	snprintf(meta_data_key, MAX_METADATA_KEY_LEN, "%s bucket of %s with value of %s consumed",
		 rd_wr, type,
		 sub_type);
	...
	snprintf(metric_name, MAX_METRIC_NAME_LEN, "QoS_per_%s_%sbucket_%s_info",
		 type, rd_wr,
		 sub_type);
	
	const metric_label_t labels[] = { lable[0], lable[1] };

	(*handle) = monitoring__register_gauge(metric_name, meta_data, labels,
					       ARRAY_SIZE(labels));

	syslog(LOG_SYSLOG,"is handler registered?");
	monitoring__gauge_set((*handle), value);
	syslog(LOG_SYSLOG,"successful register");
}


static void register_bucket_metrics(qos_class_t *qos_class, qos_bucket_t *rd_bucket, qos_bucket_t *wr_bucket, metric_label_t *label)
{
    ...
    if (bw_enable) {
        // Read bucket BW
        snprintf(max_value, QOS_MAX_LONG_STR_LEN, "%ld", rd_bucket->max_bw_allowed);
        register_metric(qos_class->type, "bw", "read",
                &(rd_bucket->bw_metric_handler), label,
                rd_bucket->data_consumed, max_value);
        // Write bucket BW
        snprintf(max_value, QOS_MAX_LONG_STR_LEN, "%ld", wr_bucket->max_bw_allowed);
        register_metric(qos_class->type, "bw", "write",
                &(wr_bucket->bw_metric_handler), label,
                wr_bucket->data_consumed, max_value);
    }
    // Register metrics for IOPS if enabled.
    if (iops_enable) {
        // Read bucket IOPS
        snprintf(max_value, QOS_MAX_LONG_STR_LEN, "%ld", rd_bucket->max_iops_allowed);
        register_metric(qos_class->type, "iops", "read",
                &(rd_bucket->iops_metric_handler), label,
                rd_bucket->iops_consumed, max_value);
        // Write bucket IOPS
        snprintf(max_value, QOS_MAX_LONG_STR_LEN, "%ld", wr_bucket->max_iops_allowed);
        register_metric(qos_class->type, "iops", "write",
    ...
    if (tokens_enable) {
        // Read bucket Tokens
        snprintf(max_value, QOS_MAX_LONG_STR_LEN, "%ld", rd_bucket->max_available_tokens);
        register_metric(qos_class->type, "tokens", "read",
                &(rd_bucket->tokens_metric_handler), label,
                rd_bucket->tokens_consumed, max_value);
        // Write bucket Tokens
        snprintf(max_value, QOS_MAX_LONG_STR_LEN, "%ld", wr_bucket->max_available_tokens);
        register_metric(qos_class->type, "tokens", "write",
...
static void register_qos_metrics(qos_class_t *qos_class)
{
	...
	if (qos_class->type == QOS_EXPORT) {
	...
	else if (qos_class->type == QOS_CLIENT) {
		struct gsh_client *client = qos_class->gsh_client;

		char client_addr[QOS_MAX_ADDR_STR_LEN];
		sprint_sockip(&(client->cl_addrbuf), client_addr, QOS_MAX_ADDR_STR_LEN);
		label[0] = METRIC_LABEL("client_Address", client_addr);
		register_bucket_metrics(qos_class, rd_bucket, wr_bucket, label);
	}

	else if (qos_class->type == QOS_PEPC) {
		syslog(LOG_SYSLOG,"I am inside qos_pepc");
		struct glist_head *glist;
		struct gsh_export *export = qos_class->gsh_export;
		...
		glist_for_each(glist, &qos_class->clients) {
			client_qos_class = glist_entry(glist, qos_class_t, clients);

			if(client_qos_class->enabled_metric)
			{
				continue;
			}
			...
			sprint_sockip(&(client_qos_class->gsh_client->cl_addrbuf), client_addr, QOS_MAX_ADDR_STR_LEN);

			syslog(LOG_SYSLOG,"client address is %s",client_addr);
			export_client_len = strlen(export->cfg_fullpath) + strlen(client_addr) +2;
			export_client = gsh_malloc(export_client_len);
			snprintf(export_client, export_client_len,"%s_%s",export->cfg_fullpath,client_addr);
			label[0] = METRIC_LABEL("export_clientid", export_client);
			syslog(LOG_SYSLOG,"now the export_cliendid is %s",export_client);
			register_bucket_metrics(client_qos_class, rd_bucket, wr_bucket, label);
			client_qos_class->enabled_metric = true;
			gsh_free(export_client);
		}
		}


	syslog(LOG_SYSLOG,"Final print of metric register");
}
...
#ifdef USE_MONITORING
	if (qos_block->qos_type == QOS_PER_EXPORT_ENABLED  && !(gsh_export->qos_class->enabled_metric)) {
		syslog(LOG_SYSLOG,"registering metrics for export");
		register_qos_metrics(gsh_export->qos_class);
		gsh_export->qos_class->enabled_metric = true;
	}
#endif
...
#ifdef USE_MONITORING
	if (!(client->qos_class->enabled_metric)) {
		register_qos_metrics(client->qos_class);
		client->qos_class->enabled_metric = true;
	}
...
/**
 * Function to consume tokens from the bucket.
 * Caller should hold qos_class->lock.
 *
 * @param [in] qos_class Pointer to the QoS client or export structure.
 * @param [in] rsize Size of the request for tokens.
 * @param [in] op_type Type of the operation (read/write).
...
/**
 * Function to defer a task for bandwidth control
 *
 ...
 * @param [in] op_type Type of the operation (read/write)
 */
static inline void qos_bw_bucket_defer_task(qos_bucket_t *bucket,
 ...
 * @param [in] caller_data Caller data passed to the callback function
 * @param [in] op_type Type of the operation (read/write)
 * @return QOS_TASK_ASYNC_SCHEDULED or NOT.
 */
static inline bool qos_process_bw(qos_bucket_t *bucket, uint64_t bytes,
				  void *caller_data, unsigned int op_type)
{
	...
	if (ctime < (last_time + required_time)) {
		...
		qos_bw_bucket_defer_task(bucket, caller_data, bytes,
 ...
 * @return QOS_TASK_ASYNC_SCHEDULED or not.
 */
static inline bool qos_control_bw(qos_class_t *qos_class, uint64_t rsize,
 ...
 * @param [in] op_type Type of the operation (read/write).
 */
static inline void qos_bw_defer_task(qos_class_t *qos_class, void *caller_data,
				     ...
				     unsigned int op_type)
{
	...
	qos_bw_bucket_defer_task(bucket, caller_data, rsize, timeout, op_type);
}
...
/**
 * Function to handle the token exhausted case and defer the task.
 * Caller should hold qos_class->lock
 *
 ...
 * @param [in] op_type Type of the operation (read/write).
 */
static void qos_token_exausted_defer_task(qos_class_t *qos_class,
					  void *caller_data,
...
static inline bool qos_check_pe_pc(qos_class_t *class_ptr, uint64_t rsize,
				   unsigned int op_type, void *caller_data,
				   compound_data_t *data)
{
	qos_class_t *qos_class = class_ptr;
	int ret = QOS_TASK_ASYNC_NOT_SCHEDULED;

	PTHREAD_MUTEX_lock(&qos_class->lock);
	if (!qos_check_token_availability(qos_class, rsize, op_type)) {
		qos_token_exausted_defer_task(qos_class, caller_data, data,
					      op_type);
		ret = QOS_TASK_ASYNC_SCHEDULED;
		goto out;
	}
...
/**
 * Function to check if the QoS token is available and consume it if possible
 * along with it takecare of defering the task to client specific Queue for BW
 *
 * @param [in] class_ptr Pointer to the QoS export object
 ...
 * @return QOS_TASK_ASYNC_SCHEDULED or not.
 */
static inline bool qos_check_pepc(void *class_ptr, uint64_t rsize,
				  unsigned int op_type, void *caller_data,
				  compound_data_t *data)
{
	qos_class_t *qos_class = class_ptr;
	...
	if (!export_token_available) {
		...
		qos_token_exausted_defer_task(sub_qos_class, caller_data, data,
					      op_type);
		PTHREAD_MUTEX_unlock(&qos_class->lock);
		return QOS_TASK_ASYNC_SCHEDULED;
	} else {
		...
		if (sub_qos_class->bw_enabled) {
			qos_bw_defer_task(sub_qos_class, caller_data, rsize,
 ...
 *			the request data.
 * @param [in] op_type Type of the operation (read/write).
 * @return : QOS_TASK_ASYNC_NOT_SCHEDULED / QOS_TASK_ASYNC_SCHEDULED
 */
unsigned int qos_process_pe(unsigned int rsize, void *caller_data,
 ...
 * @return : QOS_TASK_ASYNC_NOT_SCHEDULED/ QOS_TASK_ASYNC_SCHEDULED
 */
static inline bool qos_process_pc(unsigned int rsize, void *caller_data,
				  compound_data_t *data, unsigned int op_type)
{
 ...
 * @return : QOS_TASK_ASYNC_NOT_SCHEDULED/ QOS_TASK_ASYNC_SCHEDULED
 */
static inline bool qos_process_pepc(unsigned int rsize, void *caller_data,
				    compound_data_t *data, unsigned int op_type)
{
	syslog(LOG_SYSLOG,"ENTERED INTO qos_process_pepc");
	...
	syslog(LOG_SYSLOG,"ai am inside qos_process_pepc and bool value is %d",qos_class->enabled_metric);
	register_qos_metrics(qos_class); 
#endif
//	qos_class->type = QOS_EXPORT;
	syslog(LOG_SYSLOG,"I am back to fun 2");
	return ret;
}
 ...
 * @return : QOS_TASK_ASYNC_NOT_SCHEDULED/ QOS_TASK_ASYNC_SCHEDULED
 */
bool qos_process(unsigned int rsize, void *caller_data, compound_data_t *data,
		 unsigned int op_type)
{
	...
	switch (g_qos_config->qos_type) {
	case QOS_NOT_ENABLED:
		syslog(LOG_SYSLOG, "QOS is not enabled");
		break;
	case QOS_PER_EXPORT_ENABLED:
		syslog(LOG_SYSLOG, "QOS_PER_EXPORT_ENABLED");
		ret = qos_process_pe(rsize, caller_data, data, op_type);
		break;
	case QOS_PER_CLIENT_ENABLED:
		syslog(LOG_SYSLOG, "QOS_PER_CLIENT_ENABLED");
		ret = qos_process_pc(rsize, caller_data, data, op_type);
		break;
	case QOS_PEREXPORT_PERCLIENT_ENABLED:
		syslog(LOG_SYSLOG,"Inside the switch case");
 ...
 * caller should hold qos_class->lock to which client list belongs.
 *
				      ...
				      qos_class_t *qos_class)
{
	...
	glist_for_each_safe(glist, glistn, clients) {
		client = glist_entry(glist, qos_client_entry_t, glist);
...
{
	qos_class_t *qos_class = get_export_qos(gsh_export);
	struct glist_head *glist;
	qos_class_t *sub_qos_class;

	if (qos_class && qos_class->token_enabled) {
...
{
	qos_class_t *qos_class = get_client_qos(cl);

	if (qos_class && qos_class->token_enabled) {
		QOS_PRINT_CLIENT(__func__, cl);
		refresh_qos_token_by_class(qos_class);
	}
	/* Continue iteration */
	return true;
 ...
 * caller should hold bucket->lock.
 *
 ...
 * @param [in] op_type Type of the operation (read/write)
 */
static inline void resume_bw_io(qos_class_t *qos_class, unsigned int op_type)
{
	qos_bucket_t *bucket = qos_get_bw_bucket(qos_class, op_type);
				     ...
				     unsigned int op_type)
{
	...
	if (!bucket || ((ctime + check_delay) < bucket->bw_ldct))
		return;

	PTHREAD_MUTEX_lock(&(bucket->lock));
	glist_for_each_safe(glist, glistn, &bucket->io_waitlist_qos_bc) {
		io_entry = glist_entry(glist, timer_entry_t, glist);
		glist_del(&io_entry->glist);
 ...
 * caller should hold exports bucket->lock and clients bucket->lock.
 *
					       ...
					       uint64_t ctime)
{
	...
	uint64_t futime = 0;
	struct glist_head *glist, *glistn;
	struct glist_head *io_list = &sbucket->io_waitlist_qos_bc;

	glist_for_each_safe(glist, glistn, &cbucket->io_waitlist_qos_bc) {
		io_entry = glist_entry(glist, timer_entry_t, glist);
		rtime = (io_entry->size * 1000000) / cbucket->max_bw_allowed;

		/* Check ensures we don't exceed the Client bucket Limit */
		if (clienttime + BW_DELAY_USEC >= cbucket->bw_ldct) {
			glist_del(&io_entry->glist);
			futime = cbucket->bw_ldct + rtime + BW_EXPORT_FU_IO;
			/* Ensure full BW is available for this client
			 * else indicate export limit has been reached
			 * so client is throttling */
			if (futime > ctime) {
				cbucket->bw_ldct = cbucket->bw_ldct + rtime;
				io_entry->expiry = cbucket->bw_ldct;
			} else {
				cbucket->bw_ldct = ctime;
			...
			if (cbucket->bw_ldct > (ctime + BW_CLIENT_FU_IO)) {
					 ...
					 unsigned int op_type)
{
	uint64_t ctime = get_time_in_usec();
	qos_class_t *sub_qos_class;
	qos_bucket_t *sbucket = qos_get_bw_bucket(qos_class, op_type);
	qos_bucket_t *cbucket = NULL;
	struct glist_head *glist;
	struct glist_head *io_list = NULL;

	if (!sbucket || (ctime < sbucket->bw_ldct))
		return;

	PTHREAD_MUTEX_lock(&sbucket->lock);
	glist_for_each(glist, &qos_class->clients) {
		sub_qos_class = glist_entry(glist, qos_class_t, clients);
		cbucket = qos_get_bw_bucket(sub_qos_class, op_type);

		if (!cbucket || (ctime <= cbucket->bw_ldct))
			continue;

		io_list = &cbucket->io_waitlist_qos_bc;
		if (!glist_empty(io_list)) {
			PTHREAD_MUTEX_lock(&cbucket->lock);
			pepc_rescedule_io_to_export(sbucket, cbucket, ctime);
 ...
 */
void qos_iops_defer_task(qos_bucket_t *bucket, void *caller_data,
			 uint64_t rsize, uint64_t timeout)
{
	struct qos_op_cb_arg *qos_cb_args =
		alloc_qos_cb_args(caller_data, RATELIMITING_IO);
...
int qos_iops_check(compound_data_t *data, qos_bucket_t *bucket)
{
	...
	if (timeout <= ctime) {
		...
		qos_iops_defer_task(bucket, data, num_ops, timeout);
		return QOS_TASK_ASYNC_SCHEDULED;
	}
}
...
int qos_process_iops_pe(compound_data_t *data, uint32_t op_type)
{
	...
	if(qos_class->enabled_metric)
	{
		monitoring__gauge_set(bucket->iops_metric_handler,bucket->iops_consumed);
	}
#endif
	PTHREAD_MUTEX_unlock(&qos_class->lock);
out:
...
{
	syslog(LOG_SYSLOG,"Am i the one 1st to call?");
	qos_class_t *qos_class = op_ctx->ctx_export->qos_class;
	...
	if (qos_class->iops_enabled) {
		...
		data->qos_flags |= IS_QOS_IOPS_ACCOUNTED;
		qos_iops_defer_task(bucket, data, data->argarray_len,
				    get_time_in_usec());
#ifdef USE_MONITORING
		if(sub_qos_class->enabled_metric)
		{
			monitoring__gauge_set(bucket->iops_metric_handler, bucket->iops_consumed); 
		}
#endif
 ...
 * @return QOS_TASK_ASYNC_SCHEDULED or NOT.
 */
bool qos_process_iops(compound_data_t *data)
{
	unsigned int ret = QOS_TASK_ASYNC_NOT_SCHEDULED;
 ...
 * @param [in] op_type Type of the operation (read/write)
 */
static inline void resume_ops_io(qos_class_t *qos_class, unsigned int op_type)
{
 ...
 * caller should hold export and client bucket->lock.
 *
						 ...
						 uint64_t ctime)
{
	...
	uint64_t futime = 0;
	struct glist_head *glist, *glistn;
	struct glist_head *io_list = &sbucket->io_waitlist_qos_iops;
	uint64_t time_ops = (USEC_IN_SEC / cbucket->max_iops_allowed);

	glist_for_each_safe(glist, glistn, &cbucket->io_waitlist_qos_iops) {
		io_entry = glist_entry(glist, timer_entry_t, glist);
		...
		if (clienttime + IOPS_DELAY_USEC >= cbucket->iops_ldct) {
			...
			futime = cbucket->iops_ldct + rtime + IOPS_EXPORT_FU_IO;
			if (futime > ctime) {
				cbucket->iops_ldct = cbucket->iops_ldct + rtime;
				io_entry->expiry = cbucket->iops_ldct;
			} else {
				cbucket->iops_ldct = ctime;
				io_entry->expiry = ctime;
			}
			...
			if (cbucket->iops_ldct > ctime + IOPS_CLIENT_FU_IO) {
					...
					unsigned int op_type)
{
	...
	struct glist_head *glist;
	struct glist_head *io_list = NULL;
	qos_bucket_t *sbucket = qos_get_iops_bucket(qos_class, op_type);

	if (!sbucket || ctime < sbucket->iops_ldct)
		return;

	PTHREAD_MUTEX_lock(&sbucket->lock);
	glist_for_each(glist, &qos_class->clients) {
		sub_qos_class = glist_entry(glist, qos_class_t, clients);
		cbucket = qos_get_iops_bucket(sub_qos_class, op_type);

		if (!cbucket || ctime <= cbucket->iops_ldct)
			continue;

		io_list = &cbucket->io_waitlist_qos_iops;
		if (!glist_empty(io_list)) {
			PTHREAD_MUTEX_lock(&cbucket->lock);
			pepc_rescedule_iops_to_export(sbucket, cbucket, ctime);
			PTHREAD_MUTEX_unlock(&cbucket->lock);
		}
	}
	PTHREAD_MUTEX_unlock(&sbucket->lock);
				    ...
				    unsigned int op_type)
{
	...
	if (!bucket || ((ctime + check_delay) < bucket->iops_ldct))
		return;
...
bool ps_io_control_cb(struct gsh_export *gsh_export, void *state)
{
	...
	if (qos_class && qos_class->bw_enabled)
		resume_bw_io(qos_class, *(unsigned int *)state);

	if (qos_class && qos_class->iops_enabled)
...
static void qos_thread_init(void)
{
	...
	if (!qos_initalized) {
		...
		for (int i = 0; i < 2; i++) {
			...
			/* true since we are starting the thread */
			qos_thread_running = true;
		}
	}
	PTHREAD_MUTEX_unlock(&g_qos_iopath_lock);
}
 ...
 * @return true to continue iteration
 */
bool pe_stop_cb(struct gsh_export *gsh_export, void *state)
{
	qos_class_t *qos_class = get_export_qos(gsh_export);
 ...
 * @return true to continue iteration.
 */
bool pc_stop_cb(struct gsh_client *gsh_client, void *state)
{
	qos_class_t *qos_class = get_client_qos(gsh_client);
...
static inline void stop_qos_io(void)
{
	...
	case QOS_PEREXPORT_PERCLIENT_ENABLED:
	case QOS_PER_EXPORT_ENABLED:
		foreach_gsh_export(pe_stop_cb, false, NULL);
		break;
	case QOS_PER_CLIENT_ENABLED:
		foreach_gsh_client(pc_stop_cb, NULL);
		break;
	}
}
...
{
	LogDebug(COMPONENT_QOS, "Initating QOS shutdown");

	PTHREAD_MUTEX_lock(&g_qos_iopath_lock);
	if (!qos_initalized && !qos_thread_running) {
		PTHREAD_MUTEX_unlock(&g_qos_iopath_lock);
		return;
	}

	/* Signal threads to exit */
	qos_thread_running = false;
	...
	qos_initalized = false;
	LogDebug(COMPONENT_QOS, "Initating QOS IO cleanup");
	stop_qos_io();
	LogDebug(COMPONENT_QOS, "QOS IO Cleanup done, QOS threads stopped");
	PTHREAD_MUTEX_unlock(&g_qos_iopath_lock);
}
